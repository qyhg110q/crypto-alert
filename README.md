...
自动监控加密货币的 24 小时价格波动，当达到设定阈值时通过邮件发送告警通知。

> **最新更新**：已从 Binance API 迁移到 CoinGecko API，解决了 GitHub Actions 网络连接问题。详见 [MIGRATION.md](MIGRATION.md)

## 功能特性

- [OK] **监控主流币种**：默认监控 BTC、ETH、BNB
- [OK] **智能阈值告警**：首次 6% 告警，之后每 2% 递增（8%、10%、12%...）
- [OK] **24 小时滚动监控**：监控最近 24 小时的价格变化
- [OK] **邮件即时通知**：通过 Resend 发送告警邮件
- [OK] **GitHub Actions 部署**：无需服务器，完全托管在 GitHub
- [OK] **状态持久化**：自动保存状态，避免重复告警
- [OK] **稳定可靠**：使用 CoinGecko API，在 GitHub Actions 中表现稳定

## 工作原理

1. 定期（默认每 5 分钟）从 CoinGecko 获取市场数据
2. 获取最近 24 小时的价格变化百分比
3. 当变化幅度绝对值达到阈值时发送邮件告警
4. 每天 00:00（Asia/Shanghai）重置阈值
